export class UniqueUtils {
  public static getUniqueFields<TData, TElement = string>(
  ...
  public static getUnique<TData, TKey = string>(
    values: TData[] | undefined,
  ...
  /**
   * Создает Set с уникальными значениями. Рекомендуется использовать вместо прямого заполнения Set чтобы избегать деструктуризации (использования spread оператора).
   ...
   */
  public static createSet<TData>(
    initialValues?: Iterable<TData>,
    ...values: TData[]
  ): Set<TData> {
  ...
  public static group<TData, TKey = string | number>(
    values: TData[] | undefined,
    element: (value: TData) => TKey,
  ): Map<TKey, TData[]> {
